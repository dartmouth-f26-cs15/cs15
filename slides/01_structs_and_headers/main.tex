...
\begin{frame}[fragile]{\textpf{struct}s}
    \begin{itemize}
        ...
        \pause \item The \textpf{struct} keyword allows for the definition of ``record types," (also known as ``product types").
        \pause \item Basically, a \textpf{struct} is made by bundling together other data types.
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Populating a \textpf{struct}}
    ...
    \pause For example, to make a \textpf{struct example\_struct}, and set all of its fields to \textpf{100},
    \begin{minted}{c}
...
\begin{frame}[fragile]{Activity \#2: Fractions!}
    \begin{itemize}
        ...
        \pause \item Write a C file, \textof{fractions.c}, that defines the functions declared in \textpf{fractions.h}.
